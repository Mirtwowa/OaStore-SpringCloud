...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Insert("insert into user(username , password,create_time,updated_at,name,stu_id)"+
    ...
    @Update("update user set user.username=#{name},updated_at=#{updateTime} where username =#{username}")
    void update(User user);

    @Update("update user set user_pic=#{UserIcon},updated_at=now() where id =#{id}")
    void updateUserIcon(String UserIcon, Integer id);

    @Update("update user set password=#{md5String},updated_at=now() where username=#{account}")
